<template>
  <div class="devicerepair">
      <div class="top">
                    ...
                    <div style="flex:1">
                        ...
                        <el-col :span="4" style="margin:0 20px">
                            <el-form-item label="设备类型" prop='toType' label-width='80px'>
                                    <el-select v-model="terms.toType"   @change='changeselcet' placeholder="选择设备类型">
                                        <el-option v-for="item in deviceList" :label="item.enumValue" :key='item.enumKey' :value="item.enumKey"></el-option>
                                    </el-select>
                                </el-form-item>
                        </el-col>
                        <el-col :span="4" >
                            <el-form-item label="设备" prop='deviceId' label-width='60px'>
       ...
       <div class="titimg">
           <p>
                <span v-if='isshow'><img src="~@/assets/img/circle.png"/> 已维修</span>
                    ...
                    <el-table
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                ...
                                <!-- <img  v-if="scope.row.deviceRepairList[scope.column.index]&&(scope.row.deviceRepairList[scope.column.index].isRepairCount.split(',').findIndex(v=> v==1)!==-1)" src="~@/assets/img/circle.png"/> -->
              ...
              if(res.code==='0'){
                ...
                res.data.map((item,index) => {
                    ...
                    item.deviceRepairList.map((h,j)=>{
                        if(index=='0'){
                            this.columnlist.push({label: h.dateTime,prop:`isRepairCount${j}`})
                        }
                        // arr1[`dateTime${j}`] = h.dateTime
                        // arr1[`isRepairCount${j}`] = h.isRepairCount
                        // arr[`repairCount${j}`] = h.repairCount
                    })
                        arr.push({ ...item,...arr1})
                    });
                    console.log(arr,this.columnlist)
                    this.$nextTick(()=>{
                        this.tableData = arr
                    })
                }
